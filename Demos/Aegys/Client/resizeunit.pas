...
{ resize Source to fit in Dest
  Source,Dest bitmaps must be created and width,height set }

interface

uses windows, graphics;

procedure BMresize(Source: Tbitmap; Dest: Tbitmap);
...
// copy Source to Dest
var
  ps0, pd0, psStep, pdStep: dword; // scanline[0], row steps
...
begin
  ps0 := dword(Source.scanline[0]);
  psStep := ps0 - dword(Source.scanline[1]);
  pd0 := dword(Dest.scanline[0]);
  pdStep := pd0 - dword(Dest.scanline[1]);
  destwidth := Dest.Width - 1;
  destheight := Dest.Height - 1;
  fx := Source.Width / Dest.Width;
  fy := Source.Height / Dest.Height;
  fix := 1 / fx;
  fiy := 1 / fy;
  fxstep := 0.9999 * fx;
  fystep := 0.9999 * fy;
  ...
  begin
    ...
    begin
      ...
      destR := 0;
      destG := 0;
      destB := 0; // clear destination colors
