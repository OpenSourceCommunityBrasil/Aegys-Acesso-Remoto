...
type
  ...
  private
    ...
    aPackList: TPackList;
    Locale: TLocale;
    aConnection, aMonitor, vOldClipboardFile, vOldClipboardText: String;
    TrayWnd: HWND;
    TrayIconData: TNotifyIconData;
    BInputsBlock, TrayIconAdded, vVisualizador, isvisible: Boolean;
    ...
    procedure OnAccessGranted(Connection: String; Var ClientID, ClientPassword,
      SpecialData: String);
    ...
    procedure OnScreenCapture(Connection, ID, Command: String;
      aBuf: TAegysBytes);
...
var
  ...
  Bblockinput: Boolean;
  vResolucaoLargura, vResolucaoAltura, CF_FILE: Integer;
  mx, my: Single;
...
Begin
  ...
  Begin
    ...
    If Trim(aTempID) = '' Then
      aTempID := '0';
    MousePosX := Vcl.Forms.Screen.Monitors[StrToInt(aTempID)].Left +
      StrToInt(Copy(aLine, InitStrPos, Position - 1));
    Delete(aLine, InitStrPos, Position + 2);
    MousePosY := Vcl.Forms.Screen.Monitors[StrToInt(aTempID)].Top +
  ...
  try
    ...
    begin
      ...
      Begin
        ...
        Conexao.SendMessage(EGuestID.Text, '<|CLIPBOARD|>' + vOldClipboardText +
          '<|END|>', tctClipBoard);
      End;
    end
    else
        ...
        if s <> '' then
          ...
          begin
            ...
            Try
              ...
              vOldClipboardFile := s;
              Conexao.SendMessage(EGuestID.Text, '<|DIRECTORYTOSAVE|>' +
                FileName + '<|><|SIZE|>' + intToStr(FileStream.Size) +
                '<|END|>', tctFileTransfer);
...
Begin
  Try
    ...
    aPackList.Add(Conexao.Connection, '', tdmClientCommand, tdcAsync,
      tctScreenCapture, aBytes, Format('%s&%s&%s', [Conexao.Connection,
      Conexao.SessionID, aResolution]), True);
...
Procedure TFormConexao.OnAccessGranted(Connection: String;
  Var ClientID, ClientPassword, SpecialData: String); // tela remota
Begin
  Try
    // ResolucaoLargura
...
Procedure TFormConexao.OnPeerConnected(Connection: String;
  Var ClientID, ClientPassword, Alias: String); // Captura de tela
Begin
...
Procedure TFormConexao.OnScreenCapture(Connection, ID, Command: String;
  aBuf: TAegysBytes);
Var
  ArrayOfPointer: TArrayOfPointer;
  vStream: TMemoryStream;
  vAltura, vLargura: String;
Begin
  If Assigned(FormTelaRemota) Then
  Begin
...
Begin
  ...
  Try
    ...
    Conexao.OnÌncommingConnect := OnIncommingConnect;
