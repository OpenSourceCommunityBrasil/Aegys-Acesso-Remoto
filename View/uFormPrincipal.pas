...
begin
  ...
  begin
    ...
    i := 0;
    while i < FormPrincipal.lstConexoes.Items.Count - 1 do
    begin
      if FormPrincipal.lstConexoes.Items.Item[i].SubItems[2] = ID then
      begin
        Exists := true;
        break;
      end
...
begin
  i := 0;
  while i < FormPrincipal.lstConexoes.Items.Count do
  begin
    if (FormPrincipal.lstConexoes.Items.Item[i].SubItems[1] = ID) then
      break;

    Inc(i);
  end;
  Result := FormPrincipal.lstConexoes.Items.Item[i];
end;
...
begin
  ...
  while i < FormPrincipal.lstConexoes.Items.Count do
  begin
    if (FormPrincipal.lstConexoes.Items.Item[i].SubItems[1] = ID) and (FormPrincipal.lstConexoes.Items.Item[i].SubItems[2] = Password) then
    begin
      Correct := true;
      break;
    end;
...
begin
  ...
  begin
    ...
    memLog.Lines.Add(' ');
    memLog.Lines.Add('--------');
    memLog.Lines.Add(Header + ' (Classe: ' + ClassError + ')');
    memLog.Lines.Add('Erro: ' + MessageText);
    memLog.Lines.Add('--------');
  end;
end;
...
begin
  ...
  ssPrincipal.Port := Port;
  ssPrincipal.Active := true;

  Caption := Caption + ' - ' + GetAppVersionStr;
end;

procedure TFormPrincipal.ssPrincipalClientConnect(Sender: TObject; Socket: TCustomWinSocket);
begin
  // Create Defines Thread of Connections
  TThreadConnection_Define.Create(Socket);
end;

procedure TFormPrincipal.ssPrincipalClientError(Sender: TObject; Socket: TCustomWinSocket; ErrorEvent: TErrorEvent; var ErrorCode: Integer);
begin
  ErrorCode := 0;
end;
...
begin
  ...
  L := FormPrincipal.lstConexoes.FindCaption(0, IntToStr(mainSocket.Handle), False, true, False);
...
procedure TFormPrincipal.tLatenciaTimer(Sender: TObject);
var
  i: Integer;
  Connection: TThreadConnection_Main;
begin
  try
    for i := 0 to lstConexoes.Items.Count - 1 do
    begin
      if lstConexoes.Items.Item[i].SubItems.Objects[0] = nil then
        Continue;

      Connection := TThreadConnection_Main(lstConexoes.Items.Item[i].SubItems.Objects[0]);
      ...
      if lstConexoes.Items.Item[i].SubItems[4] <> 'Calculando...' then
        Connection.mainSocket.SendText('<|SETPING|>' + IntToStr(Connection.EndPing) + '<|END|>');
    end;
  except
    On E: Exception do
      RegisterErrorLog('Tempo de latência', E.ClassName, E.Message);
  end;
end;
